      ...
      try {
        ...
        if (stored) {
          ...
          const metricsData: MetricData = {
            ...
            facebookFollowers: dbMetrics.find(m => m.metric_type === 'facebook_followers')?.current_value || 8453,
  ...
  useEffect(() => {
    const revenueInterval = setInterval(() => {
      ...
      if (random > 0.15) {
        ...
        setMetrics(prev => {
          const newRevenue = Math.max(prev.revenue - Math.random() * 0.03, 368);
          const newMetrics = { ...prev, revenue: Number(newRevenue.toFixed(2)) };
          saveMetrics(newMetrics);
          return newMetrics;
        });
    ...
    const facebookInterval = setInterval(() => {
      ...
      setMetrics(prev => {
        let change = 0;
        if (random > 0.15) {
          change = Math.random() > 0.4 ? 2 : 1;
        } else if (random < 0.08) {
          change = -1;
        }
        const newFollowers = Math.max(prev.facebookFollowers + change, 8453);
    ...
    const viewsInterval = setInterval(() => {
      setMetrics(prev => {
        const linkedinChange = Math.floor(Math.random() * 20) - 5;
        const discordChange = Math.floor(Math.random() * 30) - 10;
        const newMetrics = {
          ...prev,
